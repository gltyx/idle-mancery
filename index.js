(function (factory) {
    ...
    const TopMenu = ({
      ...
      className: `menu-icon ${selectedTab === 'statistics' ? 'selected' : ''}`,
      src: `${HTTP_PATH}static/icons/ui/statistics.png`,
      onClick: useCiCallback(() => switchTab('statistics'))
    }), VirtualDOM.createVirtualElement("img", {
    ...
    const fmtVal = (val, notationType, radix = 2) => {
      if (!notationType) {
        notationType = BasicSettings.settings.notationTypeId;
      }
    ...
    const stringToNumber = val => {
      ...
      const testReg = /([\d.,\-]{1,})([\w]{1,5})/;
      const matches = val.match(testReg);

      if (matches && matches.length && matches.length > 2) {
        const numPart = +matches[1];
        if (Number.isNaN(numPart)) return 0;
        const suffixInd = Object.entries(SUFFIXES_ARR).find(([key, val]) => val === matches[2]);
        if (!suffixInd) return 0;
        console.log('matched: ', val, suffixInd, numPart, Math.pow(10, 3 * +suffixInd[0]));
        return numPart * Math.pow(10, 3 * +suffixInd[0]);
      }
    ...
    const FooterUI = ({
      ...
      onClick: showVersion
    }, "v0.2.5b"), VirtualDOM.createVirtualElement("p", {
      className: 'popup-link',
      onClick: showAbout
    ...
    const setPresetEdited$1 = id => {
      ...
      if (id) {
        currentPresetData = jobs.presets?.presets?.find(one => one.id === id);
      }

      if (!currentPresetData) {
        currentPresetData = jobs.presets?.defaultPreset;
      }

      console.log('open: ', id, jobs, currentPresetData);
      State.setState('game.creatures.ui.editingPreset', currentPresetData);
    };

    const changeRuleField = (id, field, value) => {
      const current = State.queryState('game.creatures.ui.editingPreset');
      const index = current.jobsRules.findIndex(one => one.id === id);

      if (index < 0) {
        current.jobsRules.push({
          id,
          [field]: value
        });
      } else {
        current.jobsRules[index][field] = value;
      }
    ...
    const toggleAutodistribute = flag => {
      console.log('toggle_autodistribute: ', flag);
      ColibriClient.sendToWorker('toggle_autodistribute', {
        flag
      });
    };
    ...
    const PresetEditor$1 = ({
      ...
      return VirtualDOM.createVirtualElement("div", {
        ...
        const foundRules = editingPreset.jobsRules?.find(one => one.id === job.id);
        return VirtualDOM.createVirtualElement("div", {
          className: 'job-preset-row'
        }, VirtualDOM.createVirtualElement("p", null, job.name), VirtualDOM.createVirtualElement("div", {
          className: 'rules'
        }, VirtualDOM.createVirtualElement("div", {
          className: 'flex rule'
        }, VirtualDOM.createVirtualElement("p", {
          className: 'label'
        }, "Set percentage"), VirtualDOM.createVirtualElement("input", {
          type: 'number',
          value: foundRules?.percentage || 0,
          onChange: useCiCallback((id, field, e) => changeRuleField(id, field, +e.target.value), [job.id, 'percentage'])
        })), VirtualDOM.createVirtualElement("div", {
          className: 'flex rule'
        }, VirtualDOM.createVirtualElement("p", {
          className: 'label'
        }, "Min amount"), VirtualDOM.createVirtualElement("input", {
          type: 'number',
          value: foundRules?.minAmount || 0,
          onChange: useCiCallback((id, field, e) => changeRuleField(id, field, +e.target.value), [job.id, 'minAmount'])
        }))));
      })));
    };
    const CreatureJobs = ({
    ...
    const SummonCreature = ({
      ...
      return VirtualDOM.createVirtualElement("div", null, VirtualDOM.createVirtualElement("div", {
        ...
        value: preset.id
      }, preset.name))), VirtualDOM.createVirtualElement("div", null, VirtualDOM.createVirtualElement("button", {
        className: 'main-action',
        onClick: reDistribute
      }, "Re-distribute")), VirtualDOM.createVirtualElement("div", null, VirtualDOM.createVirtualElement("input", {
        type: 'checkbox',
        checked: presets?.autoDistribute,
        onChange: useCiCallback(flag => toggleAutodistribute(flag), [!presets?.autoDistribute])
      }), VirtualDOM.createVirtualElement("span", null, "Auto-distribute creatures according to preset"))), VirtualDOM.createVirtualElement("p", {
        className: 'note'
      }, VirtualDOM.createVirtualElement("span", {
        className: 'title'
      }, " Warning: "), " This is experimental content. Be aware of possible bugs."))), VirtualDOM.createVirtualElement("div", {
    ...
    const Creatures = () => {
      ...
      const jobs = State.queryState('game.creatures.jobs', {});
      const editingPreset = State.queryState('game.creatures.ui.editingPreset', null);
      const editingPresetId = State.queryState('game.creatures.ui.editingPresetId', null);
    ...
    const Skills = ({
      ...
      return VirtualDOM.createVirtualElement("div", {
        ...
        className: 'flex'
      }, ['buttons', 'both'].includes(controlsSettings) ? VirtualDOM.createVirtualElement("input", {
        value: one.efforts * 100,
        onChange: useCiCallback((id, e) => changeEfforts$1(id, e.target.value), [one.id])
      }) : null), ['slider', 'both'].includes(controlsSettings) ? VirtualDOM.createVirtualElement("input", {
        type: 'range',
    ...
    const setAutomationOption = (key, value) => {
      ColibriClient.sendToWorker('set_automation_option', {
        key,
        value
      });
    };
    ...
    ColibriClient.on('set_banners_state', payload => {
      State.setState('game.banners.data', payload.list);
      State.setState('game.banners.automation', payload.automation);
    });
    const BannerRow = ({
      banner
    }) => VirtualDOM.createVirtualElement("div", {
    ...
    const BannersUI = ({
      ...
      return VirtualDOM.createVirtualElement("div", {
        ...
        className: 'banners-note'
      }, "Each banner type has several tiers. Each tier after first multiplies effect of previous one. By converting all your banners to higher tier you will loose all previous tier banners. For example, converting 2nd tier to 3rd will remove 2nd tier banners, add 5 times smaller amount to 3rd tier. Other tiers will keep the same.")), automation ? VirtualDOM.createVirtualElement("div", {
        className: 'auto-container'
      }, VirtualDOM.createVirtualElement("p", null, "Auto-banner prestige is ", automation.isTurnedOn ? 'turned on' : 'turned off', VirtualDOM.createVirtualElement("span", {
        className: 'popup-link',
        onClick: useCiCallback(() => setAutomationOption('isTurnedOn', !automation.isTurnedOn))
      }, automation.isTurnedOn ? 'turn off' : 'turn on')), VirtualDOM.createVirtualElement("div", {
        className: 'inner flex'
      }, VirtualDOM.createVirtualElement("div", {
        className: 'auto-flex-item-left'
      }, VirtualDOM.createVirtualElement("p", null, "Select banner type"), VirtualDOM.createVirtualElement("select", {
        onChange: useCiCallback(e => setAutomationOption('bannerId', e.target.value))
      }, banners.map(one => {
        return VirtualDOM.createVirtualElement("option", {
          value: one.id,
          selected: automation.bannerId === one.id
        }, one.name);
      }))), VirtualDOM.createVirtualElement("div", {
        className: 'auto-flex-item'
      }, VirtualDOM.createVirtualElement("p", null, "Set target creatures amount"), VirtualDOM.createVirtualElement("input", {
        type: 'number',
        value: automation.numCreatures,
        onChange: useCiCallback(e => setAutomationOption('numCreatures', e.target.value))
      })), VirtualDOM.createVirtualElement("div", {
        className: 'auto-flex-item-right'
      }, VirtualDOM.createVirtualElement("p", null, "Make sure you automated proper actions and using proper creatures presets. During automated run you automatically take magic courses until you have at least 50 mana.")))) : null, banners ? banners.map(one => VirtualDOM.createVirtualElement(BannerRow, {
        banner: one
      })) : VirtualDOM.createVirtualElement("p", null, "No banners available"));
    };
    const Banners = () => {
      const banners = State.queryState('game.banners.data', []);
      const auto = State.queryState('game.banners.automation', null);
      return VirtualDOM.createVirtualElement("div", {
    ...
    const importFromClipboard = () => {
      const data = document.getElementById('load-text').value;
    ...
    const hardReset = () => {
      if (confirm('This action will reset your game progress completely. You will start the game from the very beginning. Are you sure?')) {
        window.localStorage.clear();
        window.localStorage.removeItem('localSave');
    ...
    const GameSettings = ({
      ...
      return VirtualDOM.createVirtualElement("div", {
        ...
        className: 'section'
      }, VirtualDOM.createVirtualElement("h5", null, "General"), VirtualDOM.createVirtualElement("div", {
        className: 'row'
      }, VirtualDOM.createVirtualElement("div", {
        className: 'set-title'
      }, VirtualDOM.createVirtualElement("p", null, "Number Notation")), VirtualDOM.createVirtualElement("div", {
        className: 'set-setting'
      }, VirtualDOM.createVirtualElement("select", {
        onChange: useCiCallback((path, e) => switchSetting$1(path, +e.target.value), ['notationTypeId'])
      }, VirtualDOM.createVirtualElement("option", {
        value: '1',
        selected: settings.notationTypeId === 1
      }, "Standard"), VirtualDOM.createVirtualElement("option", {
        value: '2',
        selected: settings.notationTypeId === 2
      }, "Scientific"))))), VirtualDOM.createVirtualElement("div", {
        className: 'section'
        ...
        className: 'set-title'
      }, VirtualDOM.createVirtualElement("p", null, "Show input on auras page as")), VirtualDOM.createVirtualElement("div", {
        className: 'set-setting'
      }, VirtualDOM.createVirtualElement("select", {
        onChange: useCiCallback((path, e) => switchSetting$1(path, e.target.value), ['inputControls.auras'])
      }, VirtualDOM.createVirtualElement("option", {
        value: 'buttons',
        selected: settings.inputControls?.auras === 'buttons'
      }, "Input"), VirtualDOM.createVirtualElement("option", {
        value: 'slider',
        selected: settings.inputControls?.auras === 'slider'
      }, "Slider"), VirtualDOM.createVirtualElement("option", {
        value: 'both',
        selected: settings.inputControls?.auras === 'both' || !settings.inputControls?.auras
      }, "Both"))))), VirtualDOM.createVirtualElement("div", {
        className: 'section'
    ...
    const ImportExport = () => VirtualDOM.createVirtualElement("div", {
      ...
      className: 'flex-block load-block'
    }, VirtualDOM.createVirtualElement("p", null, "Import game"), VirtualDOM.createVirtualElement("div", null, VirtualDOM.createVirtualElement("textarea", {
      id: 'load-text',
    ...
    }, "Import"), VirtualDOM.createVirtualElement("input", {
      type: 'file',
      className: 'upload-file',
      onChange: uploadFile
    })))), VirtualDOM.createVirtualElement("div", {
      className: 'flex-block reset'
    }, VirtualDOM.createVirtualElement("p", null, "Hard reset (All your game data will be destroyed)"), VirtualDOM.createVirtualElement("div", null, VirtualDOM.createVirtualElement("button", {
      className: 'main-action',
      onClick: hardReset
    }, "Hard reset"))));
    const Settings = () => {
    ...
    var css_248z$5 = ".queue-wrap {\r\n    padding: 10px;\r\n    background: #030304;\r\n    margin-bottom: 10px;\r\n    overflow-y: auto;\r\n    height: 170px;\r\n}\r\n\r\n.queue-flex {\r\n    display: flex;\r\n    justify-content: flex-start;\r\n    flex-wrap: wrap;\r\n\r\n}\r\n\r\n.queue-flex .summary {\r\n    padding: 10px;\r\n    height: 120px;\r\n}\r\n\r\n.build-item {\r\n    padding: 10px;\r\n    width: 120px;\r\n    height: 120px;\r\n}\r\n\r\n.buildings {\r\n    padding: 10px;\r\n    background: #030304;\r\n}\r\n\r\n.building-wrapper {\r\n    padding: 10px;\r\n}\r\n\r\n.build-wrap {\r\n    width: 150px;\r\n}\r\n\r\n.building-wrapper .description {\r\n    width: 400px;\r\n}\r\n\r\n.embed-block {\r\n    margin-top: 10px;\r\n}\r\n\r\n.embed-block .embed-cost {\r\n    display: flex;\r\n    align-items: center;\r\n}\r\n\r\n.embed-block .embed-cost .tit {\r\n    margin-right: 5px;\r\n}\r\n\r\n.building-settings .categories {\r\n    flex-shrink: 0;\r\n}\r\n\r\n.building-settings .note {\r\n    padding: 10px;\r\n}\r\n\r\n.building-settings .note .title {\r\n    color: #f1d119;\r\n    font-weight: bold;\r\n}\r\n\r\n/*\r\n.building-settings .note .text {\r\n    font-style: italic;\r\n}*/\r\n";
    ...
    const BuildingsComponent = ({
      ...
      className: 'title'
    }, " Warning: "), VirtualDOM.createVirtualElement("span", {
      className: 'text'
    }, " All your buildings excepting megastructures will reset after banners prestige, unless you embed memory stones into them"))), VirtualDOM.createVirtualElement("div", {
      className: 'categories'
    }, categories.map(one => {
      return VirtualDOM.createVirtualElement("p", {
        className: `popup-link ${one === selectedCat ? 'selected' : ''}`,
      ...
      id: 'memoryStones'
    }), fmtVal(one.embedMemoryStoneCost)) : null, VirtualDOM.createVirtualElement("button", {
      className: 'main-action',
    ...
    const sendReRoll = (fromKey, fromIndex, bonusIndex) => {
      ColibriClient.sendToWorker('re_roll', {
        fromKey,
        fromIndex,
        bonusIndex
      });
    ...
    const chargeHeirloom = (fromKey, fromIndex, amount) => {
      ColibriClient.sendToWorker('charge_heirloom', {
        fromKey,
        fromIndex,
        amount
      });
    ...
    const setHeirloomChargeAmount = amount => {
      ColibriClient.sendToWorker('set_heirloom_charge_amount', {
        amount: stringToNumber(amount)
      });
    };
    ...
    const HeirloomsRow = ({
      ...
      return VirtualDOM.createVirtualElement("div", {
      ...
      }, items ? items.map((one, index) => {
        let delControl = null;

        if (one && key !== 'applied') {
          delControl = expanded ? VirtualDOM.createVirtualElement("button", {
            className: 'main-action',
            onClick: useCiCallback((key, index) => drop(key, index), [key, index])
          }, one.quality <= 1 ? `Drop item` : `Scrap stones & drop`) : VirtualDOM.createVirtualElement("span", {
            className: 'cross',
            onClick: useCiCallback((key, index) => drop(key, index), [key, index])
          }, "X");
        }
        ...
        }, VirtualDOM.createVirtualElement("span", null, items[index].name), items[index].quality > 1 ? VirtualDOM.createVirtualElement("span", {
        ...
        }, `x${fmtVal(items[index].quality)}`), " ") : null), expanded ? VirtualDOM.createVirtualElement("div", {
          className: 'bonuses'
        }, items[index].bonuses.map(one => VirtualDOM.createVirtualElement("p", null, one.text))) : null, delControl) : null);
      }) : VirtualDOM.createVirtualElement("p", null, "No actions available"));
    };
    const HeirloomDetails = ({
      ...
      className: 'heirloom-slot detailed'
      ...
      className: 'bonuses'
    }, selectedItem.bonuses.map((one, bonusIndex) => VirtualDOM.createVirtualElement("p", {
      className: 'stat-line'
    }, VirtualDOM.createVirtualElement("span", {
      className: 'stat-text'
    }, " ", one.text, " "), one.isRollBlocked ? VirtualDOM.createVirtualElement(Icon, {
      id: 'locked',
      scope: 'ui',
      className: 'locked-icon'
    }) : null, one.isRollAvailable ? VirtualDOM.createVirtualElement("span", {
      className: 'roll-line'
    }, " ", VirtualDOM.createVirtualElement("span", {
      className: 'do-roll popup-link',
      onClick: useCiCallback((key, index, bonusId) => sendReRoll(key, index, bonusId), [fromKey, fromIndex, bonusIndex])
    }, "Roll"), " ", VirtualDOM.createVirtualElement("span", {
      className: `cost ${one.isEnoughStones ? 'enough' : 'missing'}`
    }, VirtualDOM.createVirtualElement(ResourceIcon, {
      id: 'memoryStones'
    }), " ", fmtVal(one.rollPrice), " ")) : null))), VirtualDOM.createVirtualElement("div", {
      className: 'heirloom-foot'
    }, selectedItem.isChargeUnlocked ? VirtualDOM.createVirtualElement("div", {
      className: 'charge-block'
    }, VirtualDOM.createVirtualElement("div", {
      className: 'charge flex'
    }, VirtualDOM.createVirtualElement("button", {
      className: 'main-action smaller',
      disabled: !selectedItem.isChargeAvailable,
      onClick: useCiCallback((fromKey, fromIndex, times) => chargeHeirloom(fromKey, fromIndex, times), [fromKey, fromIndex, chargeAmount])
    }, `Charge (x${fmtVal(chargeAmount, undefined, 0)})`), VirtualDOM.createVirtualElement("input", {
      type: 'text',
      value: chargeAmount,
      onChange: useCiCallback(e => setHeirloomChargeAmount(e.target.value), [])
    }), VirtualDOM.createVirtualElement("span", {
      className: `cost ${selectedItem.isChargeAvailable ? 'enough' : 'missing'}`
    }, VirtualDOM.createVirtualElement(ResourceIcon, {
      id: 'chargeStones'
    }), " ", fmtVal(selectedItem.chargePrice), " ")), VirtualDOM.createVirtualElement("div", {
      className: 'charge charge-max flex'
    }, VirtualDOM.createVirtualElement("button", {
      className: 'main-action',
      onClick: useCiCallback((fromKey, fromIndex, times) => chargeHeirloom(fromKey, fromIndex, times), [fromKey, fromIndex, selectedItem.maxAmountChargeable])
    }, `Charge max (x${fmtVal(selectedItem.maxAmountChargeable, undefined, 0)})`), VirtualDOM.createVirtualElement("span", {
      className: `cost ${selectedItem.isChargeAvailable ? 'enough' : 'missing'}`
    }, VirtualDOM.createVirtualElement(ResourceIcon, {
      id: 'chargeStones'
    }), " ", fmtVal(selectedItem.priceForMax), " "))) : null)) : VirtualDOM.createVirtualElement("p", null, "Select item")));
    ...
    const chargeAura = (index, amount) => {
      ColibriClient.sendToWorker('charge_aura', {
        index,
        amount
      });
    };

    const setAuraChargeAmount = amount => {
      ColibriClient.sendToWorker('set_aura_charge_amount', {
        amount: stringToNumber(amount)
      });
    };
    ...
    const ActiveAuras = ({
      ...
      return VirtualDOM.createVirtualElement("div", {
        ...
        className: 'aura-rows'
      }, auras ? auras.map(one => VirtualDOM.createVirtualElement("div", null, VirtualDOM.createVirtualElement("div", {
        className: `aura-row`,
        id: `aura-row-${one.index}`
      }, VirtualDOM.createVirtualElement("div", {
        className: `aura-title`
        ...
        className: 'charged-by'
      }, `x${fmtVal(one.charge)}`), " ") : null), VirtualDOM.createVirtualElement("div", null, one.isChargeUnlocked ? VirtualDOM.createVirtualElement("div", {
        className: 'charge-block'
      }, VirtualDOM.createVirtualElement("div", {
        className: 'charge flex'
      }, VirtualDOM.createVirtualElement("button", {
        className: 'main-action',
        onClick: useCiCallback((index, times) => chargeAura(index, times), [one.index, chargeAmount])
      }, `Charge(x${fmtVal(chargeAmount, undefined, 0)})`), VirtualDOM.createVirtualElement("input", {
        type: 'text',
        value: chargeAmount,
        onChange: useCiCallback(e => setAuraChargeAmount(e.target.value), [])
      }), VirtualDOM.createVirtualElement("span", {
        className: `cost ${one.isChargeAvailable ? 'enough' : 'missing'}`
      }, VirtualDOM.createVirtualElement(ResourceIcon, {
        id: 'chargeGems'
      }), " ", fmtVal(one.chargePrice), " ")), VirtualDOM.createVirtualElement("div", {
        className: 'charge charge-max flex'
      }, VirtualDOM.createVirtualElement("button", {
        className: 'main-action',
        onClick: useCiCallback((index, times) => chargeAura(index, times), [one.index, one.maxAmountChargeable])
      }, `Charge max (x${fmtVal(one.maxAmountChargeable, undefined, 0)})`), VirtualDOM.createVirtualElement("span", {
        className: `cost ${one.isChargeAvailable ? 'enough' : 'missing'}`
      }, VirtualDOM.createVirtualElement(ResourceIcon, {
        id: 'chargeGems'
      }), " ", fmtVal(one.priceForMax), " "))) : null)), VirtualDOM.createVirtualElement("div", {
        className: 'description'
      }, VirtualDOM.createVirtualElement("p", {
        className: 'quality'
      }, "Quality: ", fmtVal(100 * one.quality), "%"), one.bonuses.map(bonus => VirtualDOM.createVirtualElement("p", null, bonus.text))), VirtualDOM.createVirtualElement("div", {
      ...
      }, "Turn off")), ['buttons', 'both'].includes(controlsSettings) ? VirtualDOM.createVirtualElement("input", {
        type: 'number',
        className: 'aura-input',
        name: `efforts-${one.index}`,
        min: "0",
        max: `${Math.min(100)}`,
        value: `${Math.round(one.effort * 10000) / 100}`,
        onChange: useCiCallback((id, e) => {
      ...
      }) : null)))) : VirtualDOM.createVirtualElement("p", null, "No active auras")));
    };
    const AllAuras = ({
      auras
    }) => {
    ...
    ColibriClient.on('set_statistics_state', payload => {
      State.setState('game.stats', payload);
    });
    const StatisticsRow = ({
      title,
      value,
    ...
    }, title), VirtualDOM.createVirtualElement("div", {
      className: 'statistics-value'
    }, value), VirtualDOM.createVirtualElement("div", {
      className: 'statistics-value'
    }, valueCurrentRun));
    const StatisticsUI = ({
      stats
      ...
      return VirtualDOM.createVirtualElement("div", {
        ...
        className: 'head'
      }, "All time")), VirtualDOM.createVirtualElement("div", {
        className: 'statistics-value'
      }, VirtualDOM.createVirtualElement("span", {
        className: 'head'
      }, "This run"))), stats.map(one => VirtualDOM.createVirtualElement(StatisticsRow, one)));
    };
    const Statistics = () => {
      const stats = State.queryState('game.stats', []);
      return VirtualDOM.createVirtualElement("div", null, VirtualDOM.createVirtualElement(StatisticsUI, {
        stats: stats
      }));
    };
    ...
    const Game = () => VirtualDOM.createVirtualElement("main", null, VirtualDOM.createVirtualElement(Header, null), VirtualDOM.createVirtualElement(Content, null), VirtualDOM.createVirtualElement(Footer, null), VirtualDOM.createVirtualElement(StoryPopup, null), VirtualDOM.createVirtualElement(Notifications, null), VirtualDOM.createVirtualElement(TemperPopup, null), VirtualDOM.createVirtualElement(ConfirmPopup, null), VirtualDOM.createVirtualElement(Modal, {
      ...
      className: 'padded'
    }, "Thanks to Triple for assistance in fixing grammar mistakes"), VirtualDOM.createVirtualElement("p", {
      className: 'padded'
    ...
    setInterval(() => {
      ...
      switch (tab) {
        ...
        case 'statistics':
          ColibriClient.sendToWorker('get_statistics_tab');
          break;
      }
    }, 200);
